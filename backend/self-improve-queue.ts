...
/**
 * Check if a proposed task overlaps with recent queue/history items.
 * Returns the overlapping item description if found, null otherwise.
 * "Recent" = last 24 hours in history + anything currently in the queue.
 * "Overlap" = ≥50% of the proposed files match an existing item's files.
 */
export function findOverlappingTask(files: string[]): string | null {
  if (!files.length) return null;

  const fileSet = new Set(files);

  // Check active queue items (pending/approved/running)
  const queue = loadQueue();
  for (const item of queue.items) {
    if (item.status === "pending" || item.status === "approved" || item.status === "running") {
      const itemFiles = item.task?.files || [];
      const overlap = itemFiles.filter(f => fileSet.has(f)).length;
      if (overlap > 0 && overlap >= Math.ceil(files.length * 0.5)) {
        return `queued:${item.id} "${(item.task?.description || "").slice(0, 60)}"`;
      }
    }
  }

  // Check recent history (last 24h)
  const history = loadHistory();
  const oneDayAgo = Date.now() - 24 * 60 * 60 * 1000;
  for (const entry of history.entries) {
    if (entry.completedAt && entry.completedAt > oneDayAgo && entry.status === "completed") {
      const itemFiles = entry.task?.files || [];
      const overlap = itemFiles.filter(f => fileSet.has(f)).length;
      if (overlap > 0 && overlap >= Math.ceil(files.length * 0.5)) {
        return `recent:${entry.id} "${(entry.task?.description || "").slice(0, 60)}"`;
      }
    }
  }
