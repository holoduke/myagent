import { getApiUrl, proxyHeaders } from '../../utils/proxy'

/**
 * Catch-all proxy for /slack/* routes (OAuth flow).
 * These routes live outside /api/ on the backend:
 *   GET /slack/workspaces
 *   GET /slack/auth/:workspaceId → returns 302 redirect to Slack OAuth
 *   GET /slack/callback         → handles OAuth callback from Slack
 *
 * We use redirect: 'manual' so 302s are passed through to the browser
 * rather than being followed server-side.
 */
export default defineEventHandler(async (event) => {
  const { pathname, search } = getRequestURL(event)
  const base = getApiUrl()
  const url = `${base}${pathname}${search}`

  try {
    const res = await fetch(url, {
      method: getMethod(event),
      headers: proxyHeaders(event),
      redirect: 'manual',
    })

    // Pass through redirects (e.g. 302 to Slack OAuth consent screen)
    if (res.status >= 300 && res.status < 400) {
      const location = res.headers.get('location')
      if (location) {
        return sendRedirect(event, location, res.status)
      }
    }

    setResponseStatus(event, res.status)

    const contentType = res.headers.get('content-type') || 'text/html'
    setResponseHeader(event, 'content-type', contentType)

    // Callback may return HTML pages
    if (contentType.includes('text/html')) {
      return res.text()
    }

    return res.json()
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err)
    console.error(`[slack] Proxy error ${url}:`, message)
    setResponseStatus(event, 502)
    return 'Service temporarily unavailable'
  }
})
